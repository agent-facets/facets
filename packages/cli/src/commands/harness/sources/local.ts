...
/**
 * Validates and resolves a local filesystem path for harness installation.
 ...
 * @returns The resolved absolute path to the harness source directory.
 */
export async function resolveLocalPath(inputPath: string): Promise<string> {
  ...
  if (!exists) {
    ...
    if (!dirExists) {
      throw new Error(`Local path "${inputPath}" does not exist or is not a harness package (no package.json found)`)
    }
  }
