...
export async function detectInstallMethod(deps?: Partial<DetectDependencies>): Promise<MethodKind> {
  ...
  const results = await Promise.all(
    probes.map(async (probe) => {
      ...
      // Case-insensitive substring match on the package name. The actual
      // output formats vary by tool (boxes, indentation, prefixes), but
      // the package name `agent-facets` is the same in all of them.
      return output.toLowerCase().includes('agent-facets') ? probe.kind : null
    }),
  )
