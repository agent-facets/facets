...
import { runCurlInstaller } from './curl.ts'
 ...
 * the new one (or vice versa); we print a warning naming both paths so
...
export const unknownMethod: InstallMethod = {
  ...
  describe: ({ targetVersion }) =>
    `(unclassified install) curl -fsSL https://agentfacets.io/install | bash -s -- --version ${targetVersion}`,
  update: async ({ targetVersion }) => {
    const runningPath = process.execPath
    process.stdout.write(`facet self-update: running binary at ${runningPath} could not be classified.\n`)
    process.stdout.write('Falling back to the curl installer (will install to ~/.facet/bin/facet).\n\n')

    // unknown method is the one place we want PATH modification: the new
    // install location may not be reachable otherwise.
    const code = await runCurlInstaller(targetVersion, { modifyPath: true })
