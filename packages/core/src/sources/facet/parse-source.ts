...
export type ParsedFacetSource = { type: 'git'; url: string; commitish?: string } | { type: 'local'; path: string }

export type ParseFacetSourceResult = { ok: true; data: ParsedFacetSource } | { ok: false; error: string }

const GITHUB_SHORTCUT_RE = /^github:([^/\s]+)\/([^/\s#]+)(?:#(.+))?$/
const LOCAL_RELATIVE_RE = /^file:(\.\.?\/.*|\.\.?)$/
const LOCAL_ABSOLUTE_RE = /^file:(\/.*)$/

export function parseFacetSource(specifier: string): ParseFacetSourceResult {
...
function parseGitUrl(raw: string): ParseFacetSourceResult {
