...
import { resolveLocalFacetSource } from '../resolve-local.ts'
...
describe('resolveLocalFacetSource', () => {
  test('resolves a relative path inside the project tree', async () => {
    const result = await resolveLocalFacetSource('./facets/viper-plans', projectRoot)
  ...
  test('resolves an absolute path inside the project tree', async () => {
    ...
    const result = await resolveLocalFacetSource(abs, projectRoot)
    expect(result.ok).toBe(true)
  })
  ...
  test('rejects a path that escapes the project tree via ..', async () => {
    ...
    try {
      ...
      const result = await resolveLocalFacetSource(relEscape, projectRoot)
  ...
  test('rejects an absolute path outside the project tree', async () => {
    const result = await resolveLocalFacetSource(outsideDir, projectRoot)
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.error).toContain('outside the project tree')
    }
  ...
  test('rejects a non-existent path', async () => {
    const result = await resolveLocalFacetSource('./facets/does-not-exist', projectRoot)
    expect(result.ok).toBe(false)
    if (!result.ok) {
  ...
  test('rejects a symlink that points outside the project tree', async () => {
    ...
    const result = await resolveLocalFacetSource('./facets/escape-link', projectRoot)
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.error).toContain('outside the project tree')
    }
  ...
  test('rejects a file (not a directory)', async () => {
    ...
    const result = await resolveLocalFacetSource('./facets/stray.txt', projectRoot)
    expect(result.ok).toBe(false)
    if (!result.ok) {
