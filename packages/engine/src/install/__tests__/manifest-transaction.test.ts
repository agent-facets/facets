...
/** A working adapter, or one whose install planning always fails. */
function installFakeAdapter(baseDir: string, name: string, opts: { failInstall?: boolean } = {}): void {
  ...
  const planBody = opts.failInstall
    ? `return { ok: false, failure: { code: 'io-failed', path: req.name, message: 'forced failure' } }`
