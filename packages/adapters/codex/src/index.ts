...
export default defineAdapter({
  ...
  normalizeForCompare(assetType, content, metadata) {
    ...
    if (assetType === 'command') {
      // Commands are installed as skills whose sidecar-routed keys
      // (`interface`/`dependencies`/`policy`) never land in SKILL.md
      // front-matter — `installCommandSkill` strips them before writing,
      // and `readAsset` reads only the SKILL.md. Strip the same keys from
      // the candidate so a re-install with those keys present still
      // compares equal (idempotent, not "repaired").
      ...
      return normalizeAssetContent(content, frontMatter)
    }
