...
Discovery SHALL construct at most two `RegistrySpec` inputs for each registry-backed manifest entry: one using the authored `VersionSpec` to resolve Target and one using `latest` to resolve Latest. It SHALL flatten those inputs in manifest order, divide them into groups of at most 100 specifiers, and invoke `resolveRegistryMetadataBatch` for every group concurrently. A full group therefore covers at least 50 facets. `resolveRegistryMetadataBatch` SHALL accept at most 100 specifiers per invocation and SHALL run its current per-specifier requests concurrently.

An **exact** authored specifier SHALL contribute only the `latest` input. A facet is checkable only when its locked version satisfies its authored specifier, and `satisfies` admits exactly one version for an exact specifier — so a pinned facet's Target *is* Current, and resolving it could return nothing else. Issuing that request anyway would make the continued availability of the installed release a precondition for updating every other facet in the project, which is precisely what the "Locked Current need not remain available" requirement forbids. Target SHALL therefore be a tagged value: a `pinned` version carrying no metadata, or a `resolved` version carrying the metadata the registry returned. A pinned Target can never advance, so nothing can select it, so it never needs a release to install.

A rejected resolver promise SHALL be converted into the same structured discovery failure a returned registry error produces. The resolver's contract is result-valued, but it is an injection point that reaches the network, and a rejection escaping discovery would surface at the command boundary as an unexplained exit `2` with no remedy attached.
...
- A checkable registry row SHALL carry the exact Target and Latest versions discovery established — with registry metadata for every version that could be installed — and SHALL be tagged as `candidate` when either version is newer than Current, otherwise as `current`. Exact pins therefore remain unchanged by default but still appear as candidates when a newer Latest can be selected interactively or with `--latest`.
- Whether a given column advances SHALL be answered by one exported engine predicate over the row's own versions, not stored beside them as a summary tag. Mode defaults, interactive selectability, and selection validation SHALL all call it. A stored summary is a second representation of a fact the versions already carry, and the failure it produces is the worst kind available here: the picker offers a row, the user confirms it, and application rejects it as a CLI defect.
...
The picker SHALL present candidate facets only and SHALL show aligned Current, Target, and Latest columns simultaneously. Each row SHALL state its chosen column as a visible word. Bold and underline MAY reinforce it but SHALL NOT carry it alone: those attributes are emitted only when the terminal advertises styling support, so under `NO_COLOR`, a pipe, or a screen reader they disappear entirely — and "which of these two similar numbers is selected" is not something a user can be left to infer. Advancing versions SHALL additionally expose the semantic size of the change by coloring only the single version component that changed — not everything from the change onward — with existing theme roles: patch uses `THEME.success` (green), minor uses `THEME.caution` (amber), and major uses `THEME.warning` (coral). Current and stationary values remain dim. Column padding SHALL sit outside the emphasized span, so a chosen cell does not trail underlined blanks. Colour is therefore never the sole carrier of any fact on this screen: the digits themselves say which component moved, and the chosen column is named in words. Reusing the existing three-rung semantic scale avoids introducing a second palette.

Both update tables SHALL derive their column widths and gutters from one shared helper, seeding each width from the column's own header label. Two implementations of "how wide is this column" is how one table ends up with a header overflowing into the next column — a defect invisible to any test that normalizes whitespace.

The picker SHALL receive a pre-filtered, non-empty list of candidates rather than the whole plan. Its cursor arithmetic has no meaning for an empty list, and "there is nothing to choose between" is a message the command owns.
...
The indicator SHALL be drawn only where a repainting region can actually live: a terminal stdout, and not CI. An animated frame is a terminal affordance; piped to a file or a CI log it becomes many frames per second in front of the output the caller wanted, and existing `--dry-run` output assertions depend on that stream staying clean. That decision SHALL come from the shared terminal-capability rule rather than a local `isTTY` test, because Ink independently applies the same CI check — and on a CI runner that allocates a pseudo-terminal the two answers diverge, leaving a mount whose `clear()` does nothing and whose `unmount()` flushes its last frame into stdout. A run that cannot repaint performs identical discovery and emits nothing extra. This is a CLI presentation wrapper only and SHALL NOT add an engine progress API or alter metadata batching. A failure to mount SHALL NOT fail the command: decoration that cannot be drawn is not a reason to abandon an update.
...
- Disclose the `facet upgrade` behavior change through the release changeset rather than by hand-editing `docs/changelog/index.mdx`. The changeset is the authored input to the release, and the public changelog entry belongs to whoever cuts it; writing both here would put two hand-maintained descriptions of one change in the repository at once, and the one in `docs/changelog/` would be dated to a release that has not happened. The changeset SHALL call out that `facet upgrade` changes from a non-mutating unimplemented stub into a live alias that applies facet updates. The historical sentence saying install shipped “without a separate `facet update` command” SHALL remain unchanged because it describes that release at that time.
...
- **[The `upgrade` alias changes an existing invocation from inert to mutating]** → Global help, the retained alias page, and the release changeset SHALL identify `update` as canonical and state that `facet upgrade` now applies the same project changes instead of printing the former stub notice.
