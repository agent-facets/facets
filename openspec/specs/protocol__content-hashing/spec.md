...
Defines how a facet-compatible system computes content hashes, assembles a deterministic two-layer `.facet` archive, derives the integrity hash, and emits the build manifest that records both. This sub-spec replaces the legacy `content-hashing` capability under the protocol scope.
...
#### Scenario: Identical inputs produce identical archives across implementations

- **WHEN** two different facet-compatible systems build the same facet from identical sources
- **THEN** the uncompressed inner tar bytes SHALL be byte-identical
- **AND** the resulting integrity hash SHALL be identical
- **AND** the archives SHALL interoperate as if produced by the same system
...
- **WHEN** a consumer receives a `.facet` archive, decompresses the inner archive using any gzip-compatible decompressor, and hashes the resulting tar bytes
- **THEN** the computed hash SHALL match the integrity hash declared in the build manifest
- **AND** the verification SHALL succeed regardless of which gzip implementation produced the compressed bytes
