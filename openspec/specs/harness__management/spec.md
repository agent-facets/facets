...
Users install, list, and remove harnesses (AI coding tool integrations) through the CLI. The system downloads harness sources from multiple source types (built-in names, npm, Git, local paths), bundles them into self-contained JavaScript files, verifies them, and stores them in a well-known directory. Installed harnesses are loaded at runtime and passed to the build pipeline so they can validate manifest metadata for their tools.
