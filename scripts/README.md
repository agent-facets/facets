...
3. **`prepack` strips `devDependencies` outright** via `stripDevDependencies` in `scripts/lib/prepack.ts` — this is the load-bearing protection that lets published packages keep `workspace:*` devDep references to versionless workspace-only packages (e.g. `@agent-facets/common` in `core` / `adapter` / `cli` devDeps). `bun pm pack` (used by the publish pipeline since PR #206) validates every `workspace:*` specifier — including those in `devDependencies` — and refuses to pack when one is unresolvable. `npm publish` strips devDeps from the tarball regardless, so deleting them at pack time has no effect on the published artifact, and `postpack` restores the original manifest. `DEP_FIELDS` excludes `devDependencies` from rewriting as belt-and-suspenders so the helper stays safe on any input shape.
