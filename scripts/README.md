...
Pre-building the tarball with `bun pm pack` and uploading it with `npm publish <filename>` makes npm derive the packument from the `package.json` *inside* the tarball — tarball and packument match by construction. The filename is parsed out of `bun pm pack --quiet`'s stdout via `extractPackFilename` (which splits on newlines, filters to `.tgz` lines, and throws on zero or multiple matches) — `bun pm pack` runs `prepack`/`postpack` lifecycle scripts inline and forwards their stdout, so naive trimming would forward a multi-line blob to `npm publish` and trip `EUNSUPPORTEDPROTOCOL` (CircleCI job 782). The companion fix is sending those lifecycle diagnostics to stderr so stdout stays single-line in practice; `extractPackFilename` is the second line of defense. The captured filename is passed to `npm publish` as a single arg, never a `*.tgz` glob — `npm publish` accepts exactly one `<package-spec>`, so a leftover tarball from a prior local pack would otherwise fail with EUSAGE.

`bun pm pack` is stricter than `npm publish` about `workspace:*` specifiers — it validates every workspace reference at pack time, including those in `devDependencies`. That's why `prepack` deletes `devDependencies` outright before pack runs (see "Workspace-only packages" above). Without that strip, packing any package with a `workspace:*` devDep on a versionless workspace-only package would fail.
