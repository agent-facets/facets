  ...
  describe('buildChangesets', () => {
    ...
    test('creates a new PR with rich body when changesets are pending', async () => {
      ...
      const loadSpy = spyOn(ci, 'loadWorkspacePackages')
        .mockResolvedValueOnce([{ name: '@agent-facets/protocol', version: '1.0.0', dir: 'packages/core' }])
        .mockResolvedValueOnce([{ name: '@agent-facets/protocol', version: '1.1.0', dir: 'packages/core' }])
