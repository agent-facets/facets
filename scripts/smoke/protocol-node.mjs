...
check('parseFacetArchive recovers the embedded build manifest', () => {
  ...
  assert.equal(result.data.buildManifest.facetVersion, 0.1)
  assert.equal(result.data.buildManifest.integrity, integrity)
  assert.equal(result.data.buildManifest.archive, 'archive.tar.gz')
})
...
check('parseFacetArchive returns ok=false on a malformed archive', () => {
  ...
  assert(Array.isArray(result.errors))
  assert(result.errors.length > 0)
  assert.equal(result.errors[0].path, 'build-manifest.json')
})
