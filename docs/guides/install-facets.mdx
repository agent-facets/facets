...
All three first-party adapters support MCP server configuration. A third-party adapter built against the older adapter API `0.1` still works fine for a project with no MCP servers, but [blocks one that has them](/cli/install#adapters-that-cannot-configure-mcp-servers).
