...
<Tip>
  Prefer a guided setup? Run `facet create my-facet` with no flags for an interactive wizard. See [`facet create`](/cli/authoring/create).
</Tip>
...
<Tip>
  Adapter-specific settings (tool lists, model preferences, permissions) live in the manifest, not the content file.

  Set them with `facet modify`:
...
A scoped identity renders as a nested path: `@acme/my-facet` at `0.0.0` writes to `dist/@acme/my-facet-0.0.0.facet`. The `.facet` file is the single distributable artifact; pass `--emit-manifest` to also write a loose `build-manifest.json` for debugging. See [`facet build`](/cli/authoring/build).
