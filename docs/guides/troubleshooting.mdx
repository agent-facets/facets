...
<AccordionGroup>
  ...
  <Accordion title='"adapter incompatible" / "unsupported adapter API" (build, add, remove, install fail before writing)'>
    **Cause:** every installed adapter declares the adapter API contract it was built against, and each CLI release supports an exact set — currently `0.1` (tagged assets only) and `0.2` (tagged assets plus MCP server configuration). A bundle installed before API versioning has **no** declaration; a bundle built against the earlier positional contract declares `0.0`, which is outside the supported set; other bundles can carry a malformed declaration, declare an API this CLI doesn't support, or disagree with the npm metadata they were selected by. All these cases fail closed: `facet build`, `facet add`, `facet remove`, and `facet install` stop **before** invoking any adapter method or writing project files.

    Membership is exact, never ordering: `0.0` is not "close enough" to `0.1`.

    <Note>
    This is a *loadability* problem. An adapter declaring `0.1` loads and works normally — it just [cannot configure MCP servers](#mcp-adapters), which is a different failure with a different fix.
    </Note>
  ...
  <Accordion title='"selected adapters cannot configure MCP servers"' id='mcp-adapters'>
    **Cause:** the project has active MCP declarations and at least one selected adapter cannot write them. The failure lists every such adapter and happens before any approval or mutation. There are two distinct cases:

    - The adapter implements adapter API `0.1`, which has no MCP support.
    - The adapter implements `0.2` but declares no MCP support.

    **Fix:** for the first, update the adapter — a newer release may support MCP:

    ```sh
    facet adapter install my-adapter
    ```

    For the second, updating will not help: that adapter has declined MCP support deliberately. Either [`facet adapter remove`](/cli/adapters/remove) it, or omit the server declarations so the project has no MCP work to do.
  </Accordion>
