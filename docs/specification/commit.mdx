...
<CardGroup cols={3}>
  ...
  <Card title="Materialize" icon="download">
    Assets written to every selected adapter, then approved MCP configuration reconciled — all journaled for rollback.
  </Card>
...
<Steps>
  ...
  <Step title="Prepare native configuration">
    Each supporting adapter parses its own native project document and computes its complete MCP change **read-only**, reporting per-server outcomes, every document the change can affect, and an opaque prepared plan. A parse or native planning conflict fails here, still with nothing written. This is also where an untracked entry already occupying a desired server name is discovered — at no extra I/O cost, because the document has just been parsed.
  </Step>
  <Step title="MCP approval">
    Every new or changed declaration, plus every untracked native entry that would be adopted or replaced, is presented as one request. Approval covers the complete set; declining ends the operation with nothing to undo, because the journal has not opened. See [MCP configuration](#mcp-configuration).
  </Step>
  <Step title="Phase C — Apply">
    The journal opens. Obsolete assets are deleted in one global pass, then every desired asset is [materialized](#materialize) under its effective name. Deleting first is what lets a name move between facets safely. An occupied destination the receipt does not own triggers a [just-in-time takeover gate](#untracked-destinations) here.
  </Step>
  <Step title="Apply native MCP configuration">
    Each prepared plan is applied as one atomic update per document, **after** all asset writes and immediately before the tri-write — the narrowest window in which a tool watching its own config could observe a server that a later failure would undo. Before each plan is applied, the byte preimages of its disclosed documents are journaled.
  </Step>
...
- **MCP entries** are disclosed *before* the journal opens, during read-only preparation. The adapter has already parsed the document to plan its change, so occupancy and equality are known for free. Declining therefore has nothing to roll back.
- **Assets** are disclosed *just in time*, during Apply, at the previous-state read that no-op detection and rollback already require. Searching the whole project for takeovers up front would be an eager scan that buys nothing; cancelling here rolls back the complete journal.
...
Configuring MCP servers requires an adapter implementing API `0.2` that declares MCP support. A selected API `0.1` adapter, or a `0.2` adapter that declares no MCP support, stays perfectly usable for a project with no active declarations. The moment one exists, every such adapter is collected into a single failure that names all of them — raised before approval and before mutation, so the project is untouched. The two cases have different remedies: an API `0.1` adapter needs upgrading, while an adapter that declares no MCP support will not gain it by being updated, so the resolution there is to omit the declarations or deselect the adapter.
...
Each adapter's contract is two operations, and the split is what makes the transaction safe:

1. **Prepare** is read-only. It parses the native document once, preserves unrelated state, detects native conflicts, computes the adapter's own rendering of each desired declaration, classifies every key, discloses every document the change can affect, and returns an opaque plan. The engine never inspects that plan.
2. **Apply** consumes the plan and performs one atomic update per document, reporting `unchanged` or the paths it changed.
...
Adapters supply **no inverse operation**. Before applying a plan, the engine journals the exact bytes of every disclosed document, so rollback restores comments, formatting, and member order exactly. Rollback fidelity never depends on adapter code being correct a second time, or on a semantic inverse reproducing a document's formatting.
...
sequenceDiagram
    ...
    Commit->>Commit: prepare native MCP change (read-only)
    Commit->>Commit: obtain MCP approval
    Commit->>Commit: Phase C — delete obsolete, then materialize
    Commit->>FS: apply prepared MCP plans (preimages journaled)
    alt all succeeded
