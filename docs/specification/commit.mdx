...
<Steps>
  ...
  <Step title="Adapter capability check">
    If the composed plan contains any active MCP declaration, every selected adapter MUST be able to configure MCP servers. Adapters that cannot are collected into **one** failure naming all of them, raised before any approval is requested and before any mutation. Each adapter also names every configuration file it planned against, and two adapters naming the same file stop the run here for the same reason: there is no order in which both plans survive. See [MCP configuration](#mcp-configuration).
  </Step>
...
Each planned change carries the document's exact inspected state, so the engine can refuse it if something else edited the file, and can put the original bytes back if a later step fails. A document the adapter inspected but does not change is not among those changes, and is therefore never journaled, never restored, and never touched. It is still **named** by the plan — a separate list that grants nothing and exists only so the engine can rule out two adapters managing one file before it asks for approval.

Every plan is recomputed immediately before its own commit, **including one that concluded nothing needed writing**. "Nothing to do" is a statement about a document, and the document may have changed since — while the approval screen was open, or by the tool itself. A second answer that differs about any server, or that was computed from a different set of files, stops the run and reports that the tool's configuration changed mid-operation. That is not an adapter defect: the adapter read its file again and reported what it found.
