...
**`facet remove`** is read-only validation, and validates one thing: that `facets.json` can be read at all. A missing or invalid manifest is a hard failure. Every requested name goes into the removals list unfiltered — **which names are actually declared is decided by commit, under the project lock**. A pre-lock read is a snapshot that a concurrent `facet add` can invalidate, and filtering against it once let a facet added a moment later survive the removal that asked for it. A name that is still absent under the lock removes nothing.
