...
| `agents`   | No       | map of string → agent descriptor  | Agent name → agent descriptor (description, prompt, harness config).         |
...
| `harnesses`   | No       | map of string → harness config | Harness name → harness-specific agent configuration.                 |
...
The `harnesses` section is OPTIONAL. It contains harness-specific configuration (tool access, permissions, model preferences) keyed by harness name. Each installed harness validates its own metadata schema at build time. Unknown harnesses produce a warning. Invalid metadata for an installed harness is a build error.
