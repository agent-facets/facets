...
3. **Validate harness metadata.** For each agent with a `harnesses` section, validate the harness-specific metadata against each installed harness's schema. Unknown harnesses SHOULD produce a warning. Invalid metadata for an installed harness MUST be a build error.
