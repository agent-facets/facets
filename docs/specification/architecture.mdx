...
3. **Harness-agnostic format.** The facet manifest format is harness-agnostic by default. Harness-specific configuration (tool access, permissions, model preferences) lives in designated extension points within the manifest. Each installed harness defines its own metadata schema — the CLI delegates validation to the harness.
