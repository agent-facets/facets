...
| **Untracked destination** | A desired effective identity already occupied by state the receipt does not own. Disclosed through a takeover gate before adoption or replacement — pre-journal for MCP entries, just-in-time for assets. | [Untracked destinations](/specification/commit#untracked-destinations) |
...
| **Prepare / apply** | The adapter's two-step MCP contract: read-only planning over the complete desired set, then one atomic update per document. Adapters supply no inverse; the engine journals byte preimages. | [Reconciliation](/specification/commit#reconciliation) |
| **Unsupported MCP adapter** | A selected adapter that cannot configure active declarations — API `0.1`, or API `0.2` declaring no MCP support. All are collected into one pre-mutation failure. | [Capability](/specification/commit#capability) |
...
| **Adapter API version** | The identifier for the adapter contract shape (currently `0.2`, which adds the MCP server capability); classified by exact match against the CLI's explicit support set, currently `{0.1, 0.2}`. One of five independent version axes, alongside the archive `facetVersion`, the project manifest version, the lockfile version, and the install-receipt version. | [Custom adapters](/guides/custom-adapters) |
