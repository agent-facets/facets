...
| **Harness**        | An AI coding tool that wraps around an LLM (e.g., OpenCode, Claude Code, Codex). The harness is the abstraction layer over its tool's storage, configuration, and asset conventions. Installed via `facet harness install`. |
