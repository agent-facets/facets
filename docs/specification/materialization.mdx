...
A name that **every** claimant aliases away from is freed the same way an omission frees one: nothing in the effective set claims it. If a previous install had materialized it, [drift removal](/specification/commit#drift-removal) deletes it in the same operation — before either alias is written — and each alias is written with its own facet's authored content.
