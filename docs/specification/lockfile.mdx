...
<ResponseField name="lockfileVersion" type="number" required>
  ...
  <Warning>
  The withdrawn closed-alpha `1` is no longer parsed. A `facets.lock` declaring it fails as an unsupported version — delete the file and run [`facet install`](/cli/install) to regenerate it. The number `1` is reserved for the eventual stable v1 schema, which is why it is not reused for a compatibility shim.
  </Warning>
</ResponseField>
...
- **Not an ownership record.** The lockfile never authorizes deletion. It is shared, version-controlled state describing what *should* be materialized; the [install receipt](/specification/commit#machine-local-install-receipt) is the sole authority on what this machine actually wrote, and an asset the receipt does not record is untracked no matter what the lockfile says. The lockfile's `assets` still describe the desired set install writes, and ownership is recorded by **effective** identity, so a name that transferred between facets is not deleted out from under its new owner. → [Drift removal](/specification/commit#drift-removal)
- **Frozen mode.** With `--frozen-lockfile`, the lockfile is the complete, authoritative source of truth *for reproduction* — never for deletion, which stays receipt-driven — and is never written. A `0.2` lockfile has no field in which to record a disposition, so a project that declares materialization overrides against one fails frozen validation rather than silently ignoring the intent. → [Frozen lockfile](/specification/commit#frozen-lockfile)
