...
- **Frozen mode.** With `--frozen-lockfile`, the lockfile is the complete, authoritative source of truth and is never written. A `0.2` lockfile has no field in which to record a disposition, so a project that declares materialization overrides against one fails frozen validation rather than silently ignoring the intent. → [Frozen lockfile](/specification/commit#frozen-lockfile)
