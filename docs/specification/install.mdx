...
When the desired state contains active MCP server declarations, every selected adapter MUST additionally support MCP server configuration. Adapters that cannot — those declaring no MCP support — MUST be collected into one failure raised before any approval is requested and before any mutation. Two selected adapters that manage the same configuration file MUST fail at the same point, naming every adapter involved: neither ordering leaves both plans applicable.
