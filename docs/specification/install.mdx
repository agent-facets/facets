...
7. **Configure servers for the active harness.** For each resolved server, generate the harness-specific configuration needed to start the server (e.g., MCP server config entries for the active AI assistant). Configuration details are handled by the installed harnesses.
...
5. **Apply selected updates.**
   ...
   - If server versions changed: download new server artifacts, verify integrity, update harness configuration.
...
- **Local disk layout specifics** — where files are placed on disk is determined by the installed harnesses. Directory mapping is a CLI concern, not a specification-level decision.
