...
- Override **groups** MUST be `skills`, `agents`, or `commands`. Any other key — `skillz`, say — is rejected, including when it sits beside a valid group. Accepting it would silently drop the intent it carries, which is the failure mode an override exists to prevent.
