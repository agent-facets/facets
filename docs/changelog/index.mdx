...
<Update label="2026-07-13" description="facet search uses server-side sorting" tags={["CLI", "Fix"]} rss={{
  title: "facet search now sorts results server-side",
  description: "facet search now uses proper server-side sorting from the registry instead of client-side filtering only, so results come back in the registry's canonical ranking order. No flag changes -- your existing facet search invocations return the same results, just correctly ordered."
}}>
  ## Fix: `facet search` uses server-side sorting

  [`facet search`](/cli/search) now defers to the registry's server-side
  sorting for result ordering, instead of applying client-side filtering
  only. Results come back in the registry's canonical ranking order --
  the same order the web UI shows -- rather than whatever order the
  registry happened to stream them in.

  No flag or argument changes. Existing invocations behave the same:

  ```sh
  facet search plan   # same query, correctly ordered results
  facet search        # same listing, correctly ordered results
  ```
</Update>
