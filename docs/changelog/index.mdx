...
<Update label="2026-07-09" description="TypeScript 7 support for SDK consumers" tags={["SDK", "Improvement"]} rss={{
  title: "TypeScript 7 support for SDK consumers",
  description: "The typescript peer dependency on the published Agent Facets SDK packages has been widened to ^5 || ^6 || ^7. If your project uses TypeScript 7 (the new native @typescript/native-preview compiler), installs no longer emit peer-dependency warnings or fail resolution. TypeScript 5 and 6 users are unaffected — no code changes required."
}}>
  ## TypeScript 7 support for SDK consumers

  The `typescript` peer dependency on the published Agent Facets SDK
  packages now accepts **`^5 || ^6 || ^7`**. If your project has adopted
  TypeScript 7 (the new native `@typescript/native-preview` compiler),
  installs no longer surface peer-dependency warnings or fail to resolve.

  TypeScript 5 and 6 users are unaffected  -- no code changes required.
</Update>


<Update label="2026-07-09" description="Generated JSON files now end with a trailing newline" tags={["CLI", "Fix"]} rss={{
  title: "Generated JSON files now end with a trailing newline",
  description: "Every JSON file the CLI writes -- facets.lock, install receipts, build manifests, scaffolded facet.json, and cache metadata -- now ends with exactly one trailing newline and uses consistent 2-space indentation. This eliminates spurious diffs when editors that auto-append a final newline open a CLI-generated file. facets.json continues to be written through the comment-preserving serializer and now upholds the same invariant."
}}>
...
<Update label="2026-07-09" description="Agent-friendly CLI: new facet instructions and facet modify commands; headless facet create; facet build --verify/--json" tags={["CLI", "New Feature"]} rss={{
  title: "Agent-friendly CLI: instructions, modify, headless create, and verify",
  description: "The facet CLI is now built for AI agents to drive non-interactively. New command: facet instructions [topic] prints agent-oriented usage guidance (overview, manifest, authoring, usage); the manifest topic emits the facet.json JSON Schema generated live from the schema definition. New command: facet modify is the scriptable counterpart to the interactive facet edit wizard — add/remove/rename/re-describe skills, agents, and commands, set facet metadata, and set per-asset adapter config with --adapter-<name> '<json>' / --remove-adapter-<name>, all with an optional --json output. facet create gains headless flags (--name, --description, --version, --private, repeatable --skill/--agent/--command, --json) that scaffold without the interactive wizard. facet build gains --verify (run the full validation pipeline without writing any output) and --json (machine-readable result). Fix: a subcommand flag named --version (e.g. facet modify facet --version 1.2.3) is no longer shadowed by the global --version flag."
}}>
  ...
  **machine-readable output**.
  ...
  hand-maintained copy. See <code>[facet instructions](/cli/instructions)</code>.

  ## New command: `facet modify`

  `facet modify` is the scriptable counterpart to the interactive
  <code>[facet edit](/cli/authoring/edit)</code> wizard: one headless, flag-driven edit
  to a facet.
  ...
  front. See <code>[facet modify](/cli/authoring/modify)</code>.
  ...
  <code>[facet create](/cli/authoring/create)</code>.
  ...
  See <code>[facet build](/cli/authoring/build)</code>.
...
<Update label="2026-06-06" description="New facet remove command (aliased rm)" tags={["CLI", "New Feature"]} rss={{
  ...
  **transactional**: any failure restores `facets.json` byte-for-byte and
  ...
  See the <code>[facet remove](/cli/remove)</code> reference for details.
</Update>
...
<Update label="2026-06-05" description="Bearer-token auth for the registry; new login/whoami/logout commands; FACET_REGISTRY_API_KEY removed; install now re-resolves a stale lockfile; new --frozen-lockfile flag" tags={["CLI", "Breaking", "New Feature", "Fix"]} rss={{
  ...
  **Fix:** `facets.json` is the source of truth, but `facet install` was
  ...
  unaffected  -- it stays pinned and reproducible. See <code>[facet install](/cli/install#lockfile-semantics)</code>.
...
<Update label="2026-05-14" description="Consolidate everything under FACET_DIR; new FACET_BIN_OVERRIDE; lock moves out of project root" tags={["CLI", "Breaking"]} rss={{
  ...
  **Breaking:** No automatic migration. The new code reads `$FACET_DIR`
  ...
  <code>[facet install](/cli/install)</code> page for the updated paths.
</Update>
