...
<Update label="2026-08-31" description="Rollback no longer leaves empty directories behind" tags={["CLI", "Fix"]} rss={{
  ...
  - **Cleanup is bounded.** The walk never removes or climbs past a
    tool's configuration directory (such as `.claude` or `.opencode`), so
    it can only reclaim the tree the install itself materialized.
