...
<Update label="2026-09-23" description="New facet outdated command and facet update --json for scripts and scheduled jobs" tags={["CLI", "New Feature"]} rss={{
  title: "New facet outdated command and facet update --json",
  description: "Two additions make facet updates scriptable. facet outdated checks every registry-backed facet in facets.json for newer releases and never writes; add --latest to compare against each facet's latest release, ignoring ranges. facet update --json prints one JSON document to stdout and nothing else, including when the command refuses to run. facet outdated --json prints the same document with applied always false. Each facet reports current, target, latest, and resolved versions, plus an outcome of updated, current, held, or unsupported. held means a newer release exists that your declared range does not allow. Check the exit code to see whether the command worked, then read applied and each outcome to see what moved. --json cannot be combined with --interactive."
}}>
  ## Check for updates without applying them

  [`facet outdated`](/cli/outdated) checks every registry-backed facet
  in `facets.json` for newer releases. It never writes to your manifest,
  lockfile, assets, or adapters.

  ```sh
  facet outdated                  # compare against each range's target
  facet outdated --latest         # compare against each facet's latest release
  ```

  ## Machine-readable update results

  [`facet update --json`](/cli/update#json-output) and
  `facet outdated --json` print one JSON document to stdout and nothing
  else, so a scheduled job or CI script can read the result directly
  instead of scraping the table.

  ```sh
  facet update --json --accept-mcp
  facet outdated --json
  ```

  - **Every facet reports its versions and an outcome.** Each entry
    lists `current`, `target`, `latest`, and `resolved`, plus an
    `outcome` of `updated`, `current`, `held`, or `unsupported`.
  - **`held` is the actionable one.** A newer release exists that your
    declared range does not allow. Run `facet update --latest` to move
    past it.
  - **Failures are JSON too.** When the command refuses to run, it
    prints an `ok: false` document with recovery details and leaves
    stderr empty.
  - **The exit code and the document answer different questions.** The
    exit code says whether the command worked. The `applied` field and
    each `outcome` say what moved.

  `--json` cannot be combined with `--interactive`.
</Update>

<Update label="2026-09-05" description="Private facets appear in facet search when signed in; clearer adapter and git authentication errors" tags={["CLI", "New Feature", "Improvement"]} rss={{
  title: "Private facets in facet search; clearer adapter and git authentication errors",
  description: "facet search now includes private facets. Signed out, results show only public facets. Signed in with facet login, results also include the private facets your account can access, ranked and paged by the registry like everything else. If a private facet you own is missing from results, run facet login and search again. Alongside open beta, the CLI's messaging dropped its alpha-era wording. Adapter compatibility diagnostics now say adapter SDK API consistently, in facet adapter list, in the failures that stop facet build, facet add, facet remove, and facet install, and in the CLI's built-in usage guidance. A git clone that needs credentials now fails with 'private repositories require git authentication' and suggests configuring git authentication or using a public URL, replacing the outdated message that claimed only public repos and SSH were supported."
}}>
  ## Private facets in `facet search` results

  [`facet search`](/cli/search) now includes
  [private facets](/private-facets). Signed out, results show only
  public facets. Signed in, results also include the private facets
  your account can access, ranked and paged by the registry like
  everything else.

  If a private facet you own is missing from results, run
  [`facet login`](/cli/login) and search again.

  ## Clearer terminology and errors

  Alongside open beta, the CLI's messaging dropped its alpha-era
  wording:

  - **Adapter compatibility errors say "adapter SDK API."** The
    diagnostics in [`facet adapter list`](/cli/adapters/list), the
    failures that stop `facet build`, `facet add`, `facet remove`, and
    `facet install`, and the CLI's built-in usage guidance all use one
    term for the [adapter SDK API](/reference/adapter-sdk) version an
    adapter declares.
  - **Git authentication failures name the real requirement.** A clone
    that needs credentials now fails with `private repositories require
    git authentication` and suggests configuring git authentication or
    using a public URL. The old message claimed only public repos and
    SSH were supported, which is no longer true.
</Update>

<Update label="2026-09-04" description="Agent Facets enters open beta with private publishing for individuals and teams" tags={["New Feature"]}>
  ## Agent Facets is in open beta

  The alpha established the full author, publish, install, and update
  loop. Agent Facets is now in open beta while we complete registry
  inspection, facet composition, and the compatibility work required for
  general availability.

  - **Public publishing remains free.**
  - **Personal private publishing is free during the beta.**
  - **Private publishing for teams is a paid offering available by
    request.**

  See the [roadmap](/roadmap) for the path to general availability and
  [Private Facets](/private-facets) for visibility, scopes, and team
  access.
</Update>

<Update label="2026-09-02" description="agent-facets 0.33.1 fixes the cold-cache crash when installing from a git source" tags={["CLI", "Fix"]} rss={{
  title: "agent-facets 0.33.1: git-sourced installs no longer crash on a cold cache",
  description: "agent-facets@0.33.1 is on npm; run facet self-update to pick it up. Installing a facet from a git source for the first time crashed with 'TypeError: Object.entries requires that input parameter not be null or undefined' instead of completing. The clone and build both succeeded; the run died on the step that commits the built content to the cache. That step is fixed, and git-sourced installs now complete on a cold cache. Only fresh clones were affected: a first install, or one whose cache slot was cold or evicted. Installs served from an audited cache hit, and facets from the registry or a local path, never hit the crash. Nothing about verification changed: the same entries are audited, and the cache records the complete set, including supplementary files that no asset owns."
}}>
  ## Git-sourced installs no longer crash on a cold cache

  The fix ships in `agent-facets@0.33.1`. Run
  [`facet self-update`](/cli/self-update) to pick it up.

  Installing a facet from a
  [git or GitHub source](/cli/add#git-and-github-sources) for the first
  time crashed with `TypeError: Object.entries requires that input
  parameter not be null or undefined` instead of completing. The clone
  and build both succeeded; the run died on the step that commits the
  built content to the cache. That step is fixed, and git-sourced
  installs now complete on a cold cache.

  - **Only fresh clones were affected.** A first install, or one whose
    cache slot was cold or evicted, hit the crash. Installs served from
    an audited cache hit, and facets from the registry or a local path,
    never took this path.
  - **Nothing about verification changed.** The same entries are
    audited, and the cache records the complete set, including
    supplementary files that no asset owns.

  If a git-sourced [`facet add`](/cli/add) or
  [`facet install`](/cli/install) failed for you with this error, update
  and run it again.
</Update>
