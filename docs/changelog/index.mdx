...
<Update label="2026-07-27" description="Resolve name collisions between facets; manifest 0.1, lockfile 0.3, and asset aliasing" tags={["CLI", "Breaking", "New Feature"]} rss={{
  ...
  **Apply**. The first two are read-only, which is why a collision, an
  ...
  Removing a facet is the exception. When the lockfile still answers
  for everything you are keeping, [`facet remove`](/cli/remove) refines
  those entries in place instead of resolving them, so it still works
  with a cold cache and an unreachable registry.
