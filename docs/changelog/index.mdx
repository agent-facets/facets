...
<Update label="2026-08-27" description="Docs site gets dedicated light and dark background colors" tags={["Improvement"]} rss={{
  title: "Docs site: dedicated light and dark background colors",
  description: "The documentation site at docs.agentfacets.io now sets its own background colors instead of relying on theme defaults: #f4f5f9 in light mode and #0a0a12 in dark mode. Pages render with the intended contrast in both modes. No action needed."
}}>
  ## Docs site: dedicated light and dark backgrounds

  The [documentation site](/) now sets its own background colors instead
  of relying on theme defaults:

  - **Light mode:** `#f4f5f9`
  - **Dark mode:** `#0a0a12`

  Pages render with the intended contrast in both modes. No action
  needed.
</Update>
