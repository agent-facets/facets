...
The CLI publishes 13 packages (12 platform binaries + 1 main package) directly to `latest` using ordering as the safety mechanism:
...
<Steps>
  ...
  <Step title="Publish main package">
  ...
  <Step title="Verify all 13 packages">
    Confirm all 13 packages (12 platform + main) are visible at the new version.
  </Step>
</Steps>

The main package is always last. Users cannot install the new version until this step succeeds, because `npm install agent-facets` resolves the main package first, which then pulls platform binaries via `optionalDependencies`. If any platform binary failed to publish, the main package never publishes, so users on the previous version are unaffected.
