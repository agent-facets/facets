...
5. **Validate harnesses** — validates harness metadata for installed harnesses (e.g., `opencode`, `claude-code`). Each harness validates its own metadata schema. Unknown harnesses produce a warning but do not fail the build.
