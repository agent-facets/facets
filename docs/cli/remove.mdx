...
Removes one or more facets from `facets.json`, deletes their assets from every connected adapter, and rewrites `facets.lock` without them  -- in a single command. The inverse of <code>[facet add](/cli/add)</code>. Aliased as `facet rm`.
...
- <code>[facet add](/cli/add)</code>  -- the inverse: add a facet to `facets.json` and install it in one step.
- <code>[facet install](/cli/install)</code>  -- reapply `facets.json` and the lockfile.