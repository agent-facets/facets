...
▸ ● viper-plans  1.2.0    1.4.0   2.0.0  (target)
...
`↑↓` moves between facets, wrapping at the ends. `◀ ▶` moves between the two columns and clamps instead, so holding one down settles on a column rather than oscillating. `l` also flips the focused row. Each row starts on its Target, or on its Latest when `--latest` is supplied.
