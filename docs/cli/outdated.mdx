...
Checks every registry-backed facet in `facets.json` against the registry and reports which ones have a newer release. It is [`facet update`](/cli/update) with previewing forced on and prompting turned off, so the two can never disagree about what would happen — there is no second implementation here to drift from the one `update` actually applies.
...
Every run behaves like a preview: nothing is ever applied, and there is no picker, because there is nothing to choose — `outdated` only reports.
