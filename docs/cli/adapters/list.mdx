...
```text
Installed adapters:
  claude-code  api 0.2      supported
  legacy-tool  api 0.1      supported
...
<Note>
`supported` means the adapter **loads**. It does not mean the adapter can configure MCP servers: `api 0.1` is a supported, asset-only contract, and a `0.2` adapter may still decline MCP support. If an install fails because an adapter [cannot configure MCP servers](/cli/install#adapters-that-cannot-configure-mcp-servers), that failure names the adapter and the right fix — this listing will show it as `supported`.
</Note>
